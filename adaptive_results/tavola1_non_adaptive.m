...
switch choiche
    ...
    case 1 % Circonferenza
        ...
        pos0 = PANDA_symplified.fkine(q0).t;
...
%         Fast Trajectory
        x = center(1) + radius * cos(3*t/t(end)*2*pi);
        y = center(2) * ones(size(x));
        z = center(3) + radius * sin(3*t/t(end)*2*pi);
        ...
        q_des= generate_trajectory2(xi,q0,PANDA_symplified)
        dq_des=gradient(q_des)*1000
        ddq_des=gradient(dq_des)*1000
        
        figure
        PANDA.plotopt = {'workspace',[-0.75,0.75,-0.75,0.75,0,1]};
        PANDA_symplified.plot(q0,'floorlevel',0,'linkcolor',orange,'jointcolor',grey)
        hold
...
grid on
PANDA_symplified.plotopt = {'workspace',[-0.75,0.75,-0.75,0.75,0,1]};
hold on
for i=1:100:length(q_des)
    
    PANDA_symplified.plot(transpose(q_des(:,i)),'floorlevel',0,'fps',1000,'trail','-k','linkcolor',orange,'jointcolor',grey)
...
Kp = 1*diag([3 3 3 3 5 3 30]);
Kv = 0.5*diag([1 1 1 1 70 2 1]);
...
for i=1:length(t)
   ...
   % Error and derivate of the error   
    ...
    C1= get_CoriolisVector(q,dq);
    M = get_MassMatrix(q);

    % Computed Torque Controller
    
    tau = ( M*(ddq_des(:,i) + Kv*(derr') + Kp*(err')) )';
...
int = 5;
n=5% intensit� percentuale della perturbazione sui parametri
for j = 1:n 
    PANDA_symplifiedmodel.links(j).m = PANDA_symplifiedmodel.links(j).m .* (1+int/100*0.5); 
end
...
pi0 = zeros(1,n*10); 
for j = 1:n
    pi0((j-1)*10+1:j*10) = [PANDA_symplifiedmodel.links(j).m PANDA_symplifiedmodel.links(j).m*PANDA_symplifiedmodel.links(j).r ...
        PANDA_symplifiedmodel.links(j).I(1,1) 0 0 PANDA_symplifiedmodel.links(j).I(2,2) 0 PANDA_symplifiedmodel.links(j).I(3,3)];
end
piArray(1,:) = pi0; 
...
for i = 2:length(t)
    ...
    if sel2 == 2||sel2 == 3|| sel2 == 4 
        for j = 1:n 
            PANDA_symplifiedmodel.links(j).m = piArray(i-1,(j-1)*10+1); % elemento 1 di pi
        end
    end
    
    Mtilde = PANDA_symplifiedmodel.inertia(q(i-1,:)); 
    Ctilde = PANDA_symplifiedmodel.coriolis(q(i-1,:),q_dot(i-1,:)); 
    Gtilde = PANDA_symplifiedmodel.gravload(q(i-1,:)); 
...
switch sel2
    ...
    M = PANDA_symplifiedmodel.inertia(q(i-1,:)); 
    C = PANDA_symplifiedmodel.coriolis(q(i-1,:),q_dot(i-1,:)); 
    G = PANDA_symplifiedmodel.gravload(q(i-1,:)); 
